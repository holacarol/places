...
describe Place do
  ...
  describe "validations" do
	...
	:address_attributes => @address_attr, :phone_number => "915 34 27 84", :url => "http://www.testplace.com", 
  :author_id => @tie.receiver.id,	:owner_id => @tie.sender.id, :user_author_id => @tie.receiver.id }
  ...
  :author_id => @tie.receiver.id, :owner_id => @tie.sender.id, :user_author_id => @tie.receiver.id }
	...
	:address_attributes => @address_attr, :phone_number => "915 34 27 84", :url => "http://www.testplace.com", 
  :author_id => " ", :owner_id => @tie.sender.id, :user_author_id => @tie.receiver.id }
      Place.new(@wrong_place).should_not be_valid
    end
    ...
    it "should require nonblank owner_id" do
      @wrong_place = {:title => "Test place", :latitude => "40.4166909", :longitude => "-3.7003454",
	:address_attributes => @address_attr, :phone_number => "915 34 27 84", :url => "http://www.testplace.com", 
  :author_id => @tie.receiver.id,	:owner_id => " ", :user_author_id => @tie.receiver.id }
      Place.new(@wrong_place).should_not be_valid
    end
