class SearchController < ApplicationController
  ...
  def index
    ...
    respond_to do |format|
      format.html {
        ...
        elsif params[:mode] == "place_search"
  ...
  def search mode
    ...
    if mode.to_s.eql?("place")
      result = ThinkingSphinx.search(get_search_query, :classes => models, :order => :like_count, :sort_mode => :desc)
    else
      result = ThinkingSphinx.search(get_search_query, :classes => models)
    end
    result = authorization_filter result
    if mode.to_s.eql? "quick" || mode.to_s.eql?("place")
